...
<head>
  ...
  header("Content-type: text/plain"); //Set the content type to plain text so the print below is easy to read!
	...
	//	print_r($original_file);
		...
		for($i = 0;$i < sizeof($matches[0]);$i++){
			...
			if($hadFoodItem){
				...
				if($isVG == true) $food_array[3][$food_info_index-1] = 1;
	...
	//Parse a nutrition value expressed as a fraction into a decimal. If the value is not a fraction, do nothing.
	function parse_nutrition_fractions($frac){
	
		if(!strstr($frac, '\/'))
			return $frac;
		
		print_r("Nutrition fraction:" . $frac . "\n");

		$numerator = strstr($frac, '\\', true)

		print_r("Nutrition numerator:" . $numerator . "\n");

		$denom = strstr($frac, '\/');
		$denom = substr($frac, 1);

		print_r("Nutrition denomenator:" . $denom . "\n");

		$dec = floatval($numerator)/floatval($denom);
		$dec = round($dec, 2);

		print_r("Decimal value:" . $dec . "\n\n");
	...
	function populate_has_img($site){

		$stripped_file = strip_tags($site, "<div><a>");
		preg_match_all("/div class=\"field\-content\">(.*?)<\/div>/s", $stripped_file, $divs);
		print_r("Divs Array:" . $divs . "\n");
		print_r("div strippedfile:" . $strippled_file);
	...
	$original_file = file_get_contents("http://www.dining.columbia.edu/menus/67");
	...
	print_r("Before Update" . "\n");	

	update_SQL_table($food_arrays, $image_arrays, $nutrition_arrays);
	
	print_r("Dont updating \n");

	$abc = populate_has_img($original_file);
