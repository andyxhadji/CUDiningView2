...
<form action="<?php echo url_for('jjsplace/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?food_id='.$form->getObject()->getFoodId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
    ...
    <tfoot>
      <tr>
        <td colspan="2">
          ...
          &nbsp;<a href="<?php echo url_for('jjsplace/index') ?>">Back to list</a>
          <?php if (!$form->getObject()->isNew()): ?>
            &nbsp;<?php echo link_to('Delete', 'jjsplace/delete?food_id='.$form->getObject()->getFoodId(), array('method' => 'delete', 'confirm' => 'Are you sure?')) ?>
          <?php endif; ?>
